...
const Home = () => {
  ...
  const validateFileByMagicBytes = async (file) => {
    try {
      ...
      const isAvif =
        file.name.toLowerCase().endsWith(".avif") || file.type === "image/avif";
  ...
  const processFiles = useCallback(
    async (files) => {
      ...
      if (!validFiles.length) {
        if (files.length > 0)
          showToast(
            "Only JPG, PNG, WEBP, AVIF, SVG, and HEIC formats are supported.",
          );
        return;
      }
      ...
      if (validFiles.length > slotsAvailable) {
        showToast(
          `Limit is ${MAX_IMAGES} images. Only adding the first ${slotsAvailable} file(s).`,
        );
      ...
      for (let file of validFiles) {
        ...
        try {
          ...
          // Persist to DB
          saveImageToDB(newImgObj).catch((err) =>
      ...
      // Alerts
      let alertMsg = "";
      if (maxRejectedCount > 0)
        alertMsg += `${maxRejectedCount} file(s) skipped > 30MB.\n`;
      if (minRejectedCount > 0)
        alertMsg += `${minRejectedCount} file(s) skipped < 5KB.\n`;
      if (invalidFormatCount > 0)
        alertMsg += `${invalidFormatCount} file(s) skipped - invalid or corrupted format.\n`;
      if (processingErrorCount > 0)
        alertMsg += `${processingErrorCount} file(s) failed to load (unknown error).\n`;
  ...
  const processAndDownloadImage = async (imgData, signal) => {
    ...
    if (mode === "size") {
      ...
      if (targetSizeBytes < 5 * 1024) {
        return {
          success: false,
          error: `Skipped ${imgData.name}: Target size < 5KB.`,
        };
      }
      // 2. Max Size Check
    ...
    try {
      ...
      if (
        mode === "quality" &&
        imgData.targetQuality === "max" &&
        tFormat === imgData.type
      ) {
        // Fetch original blob
        ...
        processedBlob = await processImageToSize(
          imgData,
          targetSizeBytes,
          tFormat,
          signal,
  ...
  const processImageToSize = async (
    ...
    if (mode === "quality") {
      ...
      return new Promise((resolve) =>
        canvas.toBlob(resolve, outputType, fixedQuality),
      );
    }
    ...
    if (!bestBlob && outputType === "image/avif") {
      ...
      canvas.width = 1;
      canvas.height = 1;
      const testBlob = await new Promise((r) =>
        canvas.toBlob(r, "image/avif", 0.5),
      );
    ...
    <div className={styles.container}>
      ...
      <Helmet>
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            url: "https://www.img-reducer.com",
            description:
              "Free online image resizer, compressor, and converter.",
      ...
      <main>
        ...
        <div className={styles.info}>
          <span>Supported formats: JPG/JPEG, PNG, SVG, WebP, HEIC, AVIF</span>
          <div>
            <span className={styles.info_secondary}>
              Insert limit: 10 images,
            </span>
            <span className={styles.info_secondary}>Max: 30MB,</span>
            <span>Min: 5KB</span>
          </div>
        </div>

        {warningMsg && <div className={styles.warningToast}>{warningMsg}</div>}
          ...
          >
            ...
            {images.length === 0 && !isProcessing && (
              <div className={styles.placeholder} onClick={handleBrowse}>
                <img
                  src="/drop-zone_image.png"
                  alt="drop-zone"
                  className={styles.dropZone_Image}
                />
                <p className={styles.dropZoneText}>Load Images</p>
                <span className={styles.dropZoneTextTwo}>
                  Drag & drop, paste, or click to
                </span>
                <span className={styles.dropZoneTextThree}>browse</span>
              </div>
            )}
